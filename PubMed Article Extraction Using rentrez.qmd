...
**Notes pre-search:** No MeSH terms. Looked to literature on the subject to find relevant terms:
...
# Define the search term:
search_term_work <- '(((("work addiction"[Title/Abstract]) OR ("work dependence"[Title/Abstract])) OR ("compulsory work"[Title/Abstract])) OR ("obligatory work"[Title/Abstract])) OR ("Addiction to work"[Title/Abstract])'
...
# Split id list into chunks of 500:
chunks <- split(id_list_work, ceiling(seq_along(id_list_work)/100))
...
```{r}
# Add search URL:
url_work_rvest <- "https://pubmed.ncbi.nlm.nih.gov/?term=%28%28%22work+addiction%22%5BTitle%2FAbstract%5D%29+OR+%28%22addiction+to+work%22%5BTitle%2FAbstract%5D%29%29+OR+%28%22workaholism%22%5BTitle%2FAbstract%5D%29&size=200" # 275 results returned on 20/07/2023
...
work_hist<- hist(results_work2$Year,
                     xlim = c(1960,2023),
                     breaks = 10,
                     main = "Papers published per year on work Disorder (PubMed)",
                     xlab = "Year")
...
# View(results_work2) # Visually inspect results
...
Label <- rep("work", times = count(results_work2))

results_work_final <- results_work2 %>% bind_cols(Label) %>%
  rename(Label = 14) %>%
  print()
