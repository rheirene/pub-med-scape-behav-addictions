...
packages <- c("tidyverse", 
              ...
              "osfr",
              "readxl",
              "httr", # supports automation of geolocation
               "jsonlite", # supports automation of geolocation
...
# Paraguay:
str_detect(institution, "(?i)Jesus University") ~ "Asuncion",
...
# UK
str_detect(institution, "(?i)University of Warwick") ~ "Coventry",
...
# china:
 str_detect(institution, "(?i)Beijing University") ~ "Beijing",
...
# Poland:
 ...
 str_detect(institution, "(?i)Adam Mickiewicz University") ~ "Poznan",
  ...
  # Brazil:
str_detect(institution, "(?i)Mackenzie Presbyterian") ~ "Sao Paulo",
...
str_detect(institution, "(?i)Sahar Obeid") ~ "Lebanon", # This is an author I looked up
  # Brazil:
str_detect(institution, "(?i)Mackenzie Presbyterian") ~ "Brazil",
...
str_detect(institution, "(?i)University College, Bergen") ~ "Norway",
  # South Korea:
str_detect(institution, "(?i)Hanyang University") ~ "South Korea",
str_detect(institution, "(?i)Yonsei University") ~ "South Korea",
str_detect(institution, "(?i)Department of Nursing, Chung-Ang") ~ "South Korea",
str_detect(institution, "(?i)Anyang University") ~ "South Korea",
str_detect(institution, "(?i)Chungbuk National University") ~ "South Korea",
str_detect(institution, "(?i)Chungmugong Leadership Center") ~ "South Korea",
  str_detect(institution, "(?i)Pusan National University") ~ "South Korea",
 str_detect(institution, "(?i)Honam University") ~ "South Korea",
str_detect(institution, "(?i)Korea Institute on Behavioral Addictions") ~ "South Korea",
str_detect(institution, "(?i)hallym University") ~ "South Korea",
str_detect(institution, "(?i)Hallym University") ~ "South Korea",
str_detect(institution, "(?i)Seoyoung University") ~ "South Korea",
str_detect(institution, "(?i)Yonsei Forest Mental Health Clinic") ~ "South Korea",
...
str_detect(institution, "(?i)IAJ (Gambling Support Institute)") ~ "Portugal",
 ...
 str_detect(institution, "(?i)Beijing University") ~ "China",
...
# Now look at papers where we couldn't match the country but linking the world.cities dataset connects a country with a city. Error check this:
 ...
 # 2025-06-09: Need to re-look at these now, slowly. Tell ChatGPT to ensure it's checking for mathcing cities and not towns
 
 # 2025-06-25 DID THE BELOW FROM THE BOTTOM UP TO 850
...
# Prepare finalised location data:
data_locations_with_full_geo_location_cleaned_final <-   data_locations_with_full_geo_location_cleaned %>%
   # select(institution,
   #       cities_of_authors,
   #       countries_of_authors,
   #       country.etc) %>%
    # Add in the missing country where it is available from the world.cities dataset, as per the last recoding check above:
    mutate(countries_of_authors = case_when(is.na(countries_of_authors) ~ as.character(country.etc),
    TRUE ~ countries_of_authors)) %>%
    mutate(countries_of_authors = case_when(
    str_detect(countries_of_authors, "Korea South") ~ "South Korea",
     str_detect(countries_of_authors, "Korea North") ~ "North Korea",
    TRUE ~ countries_of_authors)) 
  

  
# NEXT: need to try and link long and lat to the combination of city and country where possible
...
```{r}
# NEED TO RUN IN CONSOLE FIRST! Not included here to avoid commiting this information to a public repository 


# Sys.setenv(OPENAI_API_KEY = "")

correct_city <- function(institution, cities_of_authors) {
  prompt <- paste0(
    "Institution: '", institution, 
  "'. Detected city: '", cities_of_authors, 
  "'. I want the location of this institution. Respond ONLY with the name of the nearest city (no explanation) and the country that city is based in, like this: 'City name, Country name'. If the detected city is already the correct city, repeat it exactly. Do not give the name of a town or county, only a city. If you are unsure about the city or country name, just respond NA in place of either"
)
  ...
  res <- POST(
    ...
    content_type_json(),
    body = toJSON(list(
      model = "gpt-3.5-turbo", # MUCH cheaper than using gpt-4.5
...
safe_correct_city <- possibly(correct_city, otherwise = NA_character_) # this is cool, this funtion basically tells it to just proceed if there's an error in one row rather than freaking out

```

Test the function on random samples of 200 affiliation rows to see how it does:
```{r}

# TEST RUN:
automation_data<- data_locations_with_full_geo_location_cleaned_final %>% slice(1:200)

institution_data <- automation_data %>%
  rowwise() %>%
 mutate(city_corrected = list({
  Sys.sleep(1.1)
  safe_correct_city(institution, cities_of_authors)
}))
...
institution_data %>%
  select(institution, 
         cities_of_authors,
         city_corrected) %>%
  unnest(city_corrected) %>%
  print(n=200)
```

Wow, this is so much better than mapping against lists of city/country names!

```{r}
batch_size <- 100
n_batches <- ceiling(nrow(data_locations_with_full_geo_location_cleaned_final) / batch_size)
...
for (i in seq_len(n_test_batches)) {
  start_idx <- ((i - 1) * batch_size + 1)
  end_idx <- min(i * batch_size, nrow(data_locations_with_full_geo_location_cleaned_final))
  
  message("Processing batch ", i, " of ", n_batches, " (rows ", start_idx, "-", end_idx, ")")
  
  batch <- data_locations_with_full_geo_location_cleaned_final[start_idx:end_idx, ]
  
  result <- process_batch(batch)
  
  # Save partial result (optional but recommended)
  write.csv(result, paste0("city_check_batch_", i, ".csv"), row.names = FALSE)
...
write.csv(final_result, "city_corrected_all.csv", row.names = FALSE)

```


Process and finalise location data:
```{r}

# Add Hong Kong to world.cities dataset, with approximate values
hong_kong_row <- data.frame(
  name = "Hong Kong",
  country.etc = "Hong Kong",  
  pop = 7500000,
  lat = 22.3193,
  long = 114.1694,
  capital = 1
...
world.cities_extended <- bind_rows(world.cities, hong_kong_row)

world.cities_extended %>%
  filter(str_detect(name, "Hong Kong"))

# Normalize names to improve matching
cities_clean <- world.cities_extended %>%
  mutate(
    city_lower = tolower(name),
    country_lower = tolower(country.etc)
  )

data_locations_with_full_geo_location_cleaned_final_normalised <- data_locations_with_full_geo_location_cleaned_final %>%
  mutate(
    city_lower = tolower(cities_of_authors),
    country_lower = tolower(countries_of_authors)
  )


location_data <- data_locations_with_full_geo_location_cleaned_final_normalised %>%
  select(-lat,
         -long) %>%
  left_join(cities_clean, by = c("city_lower", "country_lower")) %>%
  select(-city_lower, -country_lower)

location_data %>% select(institution, 
                         cities_of_authors,
                         countries_of_authors,
                         lat,
                         long) %>%
...
data_locations_with_full_geo_location_WORLD<- left_join(location_data, world_map) %>%
...
all_publications_map <- ggplot(world_map, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill="#FAFFFF", colour = "grey",
               ) +
   geom_point(data = data_locations_with_full_geo_location_WORLD_city_aggregate,
            ...
            #  size = sqrt(num_papers),
              ...
              fill = "#289998",
              alpha = 0.4,
              shape = 21) +  # shape 21 for filled circles
  ...
  labs(
    # title = "Institution locations of authors publishing\n behavioural addictions research",
    # subtitle = "Each circle represents a city and its size corresponds\n to the number of papers linked to it",
    caption = "",
    size     = "No. publications"
  )

# Save this now as a basic plot:
ggsave("Figures/all_publications_map.pdf",
...
# Convert static plot to ggplotly format and adjust theme settings where required:
 ...
 list(x = 1, y = -0.1, text = "Rob Heirene",
...
```{r}
# ALSO CHECK HONG KONG!

# 1. Summarise publications by country:
country_pubs <- data_locations_with_full_geo_location_WORLD %>%
  filter(!is.na(countries_of_authors)) %>%
  group_by(countries_of_authors) %>%
  summarise(num_papers = n_distinct(PMID), .groups = "drop")

# 2. Prepare a world map frame
world_map <- map_data("world") %>% 
  filter(region != "Antarctica")

# 3. Join the publication counts to the map data:
choropleth_map <- world_map %>%
  left_join(country_pubs, by = c("region" = "countries_of_authors"))

#Love this:
# Find a lighter color to contrast our darkest shade:
# library(colorspace)
# 
# colorspace::lighten("#289998", 0.99) # Make this **closer to 1** for lighter shades, ROB!

# 4. Plot:
all_publications_choropleth <- ggplot(choropleth_map, aes(x = long, 
                                                             y = lat, 
                                                             group = group,
                                                             fill = num_papers)) +
 ...
 plot_theme +
  theme(
    ...
    axis.line.x = element_blank(),
    axis.line.y = element_blank(),
    # plot.margin = margin(-20,0,-1,-30)
    ) +
  labs(
    # title    = "Publications per country",
    # subtitle = "",
    caption  = ""
  ) 

all_publications_choropleth

# 5. Save:
ggsave("Figures/all_publications_choropleth.pdf",
       plot = all_publications_choropleth,
...
# 4. Plot:
gambling_publications_choropleth <- ggplot(choropleth_map, aes(x = long, y = lat, group = group, fill = num_papers)) +
  geom_polygon(colour = "black", size = 0.2) +
  coord_fixed(1.3) +
  scale_fill_gradient(
    name    = "Count",
 ...
 plot_theme +
  ...
  labs(
    title    = "Gambling",
    # subtitle = "Shaded by total number of behavioural addictions papers",
    caption  = ""
  ) 

gambling_publications_choropleth

# 5. Save:
ggsave("Figures/gambling_publications_choropleth.pdf",
       plot = gambling_publications_choropleth,
       width = 10, height = 5.5, dpi = 600)
```
...
# 4. Plot:
gaming_publications_choropleth <- ggplot(choropleth_map, aes(x = long, y = lat, group = group, fill = num_papers)) +
  geom_polygon(colour = "black", size = 0.2) +
  coord_fixed(1.3) +
  scale_fill_gradient(
    name    = "Count",
...
gaming_publications_choropleth

# 5. Save:
ggsave("Figures/gaming_publications_choropleth.pdf",
       plot = gaming_publications_choropleth,
       width = 10, height = 5.5, dpi = 600)
```
...
# 4. Plot:
internet_publications_choropleth <- ggplot(choropleth_map, aes(x = long, y = lat, group = group, fill = num_papers)) +
  geom_polygon(colour = "black", size = 0.2) +
  coord_fixed(1.3) +
  scale_fill_gradient(
    name    = "Count",
...
internet_publications_choropleth

# 5. Save:
ggsave("Figures/internet_publications_choropleth.pdf",
       plot = internet_publications_choropleth,
       width = 10, height = 5.5, dpi = 600)
```
...
# 4. Plot:
smart_phone_publications_choropleth <- ggplot(choropleth_map, aes(x = long, y = lat, group = group, fill = num_papers)) +
  geom_polygon(colour = "black", size = 0.2) +
  coord_fixed(1.3) +
  scale_fill_gradient(
    name    = "Count",
...
smart_phone_publications_choropleth

# 5. Save:
ggsave("Figures/smart_phone_publications_choropleth.pdf",
       plot = smart_phone_publications_choropleth,
       width = 10, height = 5.5, dpi = 600)
```
...
```{r}
combined_unique_types_publications_choropleth<- (gambling_publications_choropleth + gaming_publications_choropleth)/
  (internet_publications_choropleth + smart_phone_publications_choropleth) 

combined_unique_types_publications_choropleth

# 5. Save:
ggsave("Figures/combined_unique_types_publications_choropleth.pdf",
       plot = combined_unique_types_publications_choropleth,
       width = 10, height = 5.5, dpi = 600)
```
