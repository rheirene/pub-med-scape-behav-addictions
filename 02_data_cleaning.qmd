...
str_detect(institution, "Toronto, ON") ~ "Toronto",
    str_detect(institution, "Carleton University") ~ "Ottawa",
  # Indian city:
  str_detect(institution, "Veer Chandra Singh Garhwali Government") ~ "Srinagar",
 str_detect(institution, "Anna Nagar") ~ "Chennai",

   # Hong kong:
   str_detect(institution, "Hong Kong") ~ "Hong Kong", 
 str_detect(institution, "Tung Wah Group") ~ "Hong Kong", 
...
str_detect(institution, "Universidad del Pais Vasco") ~ "San Sebastian",
  # France/french:
    str_detect(institution, "University of Tours") ~ "Tours",
    str_detect(institution, "CSAPA, pole ressource jeux") ~ "Lille",
str_detect(institution, "Universite de Versailles Saint Quentin en Yvelines") ~ "Le Chesnay",
str_detect(institution, "Centre Hospitalier Sainte-Anne") ~ "Paris",
  ...
  str_detect(institution, "Centre Pierre Nicole") ~ "Paris",
    str_detect(institution, "centre hospitalier Sainte-Anne") ~ "Paris",
    str_detect(institution, "Center Pierre Nicole") ~ "Paris",
  # South Korea:
  str_detect(institution, "Seoyoung University") ~ "Paju-si",
  str_detect(institution, "Chungbuk National University") ~ "Cheongju",
 ...
 # Johns Hopkins University:
    str_detect(institution, "Johns Hopkins University") ~ "Baltimore",
  ...
  str_detect(institution, "Edith Nourse Rogers") ~ "Bedford",
# Florida:
 str_detect(institution, "University of Florida") ~ "Gainesville",
# Yale:
  str_detect(institution, "New Haven, CT") ~ "New Haven",
  str_detect(institution, "Yale University") ~ "New Haven",
# US unis:
 str_detect(institution, "Columbia University") ~ "New York",
	 str_detect(institution, "University of Minnesota") ~ "Minneapolis",
str_detect(institution, "University of Rhode Island") ~ "Kingston",
...
str_detect(institution, "South Limburg Mental Health") ~ "Netherlands", 
...
str_detect(institution, "Bar-Ilan University") ~ "Ramat Gan",

# South Korea:
str_detect(institution, "Hanyang University") ~ "",
str_detect(institution, "Yonsei Forest Mental Health Clinic") ~ "Seoul",
...
# Perth/Australia:
 ...
 str_detect(institution, "University of Wollongong") ~ "Wollongong", 
 str_detect(institution, "Adelaide") ~ "Adelaide", 
str_detect(institution, " CQUniversity, 400 Kent St, Sydney") ~ "Sydney", 
str_detect(institution, " CQ University") ~ "Rockhamptom", 
# Oxford Uni:
...
str_detect(institution, "Vincent P. Dole Dual Diagnosis Unit") ~ "Lucca", 
str_detect(institution, "Institute for Advanced Studies-IUSS") ~ "Pavia", 

  # barcelona & other spanish city issues:
  str_detect(institution, "Barcelona, Spain") ~ "Barcelona",
  str_detect(institution, "Pompeu Fabra University") ~ "Barcelona",
...
str_detect(institution, "Servicio de Psiquiatria, Fundacion") ~ "Mardrid",
...
str_detect(institution, "Abel Salazar") ~ "Porto",

str_detect(institution, "IAJ (Gambling Support Institute)") ~ "Lisbon",
str_detect(institution, "Rua Sacadura Cabral") ~ "Lisbon",
# Turkey:
...
str_detect(institution, "William Paterson University") ~ "Newark",
...
str_detect(institution, "Tokai Gakuen University") ~ "Nagoya",
...
str_detect(institution, "Nguyen Tat Thanh") ~ "Ho Chi Minh City",
   # Santo André:
...
str_detect(institution, "Cambridge Health Alliance") ~ "Malden",
str_detect(institution, "South Limburg Mental Health") ~ "Maastricht", 
...
str_detect(institution, "University of Essex") ~ "UK",
   str_detect(institution, "Johns Hopkins University") ~ "USA",
   str_detect(institution, "University of Massachusetts") ~ "USA",
str_detect(institution, "University of Rhode Island") ~ "USA",
...
str_detect(institution, "Clementino Fraga Filho Hospital") ~ "Brazil",
    ...
    str_detect(institution, "Universidade Federal de Minas Gerais") ~ "Brazil",
...
str_detect(institution, "Universidade Estadual da Paraiba") ~ "Brazil",
    ...
    str_detect(institution, "CSAPA, pole ressource jeux") ~ "France",
    # Poland: 
...
str_detect(institution, " CQ University") ~ "Australia",
  str_detect(institution, " University of Wollongong") ~ "Australia",
  ...
  # Canada:
    str_detect(institution, "Department of Education, Centre for Addiction and Mental Health") ~ "Canada",
      ...
      str_detect(institution, "Brandon University") ~ "Canada",
...
str_detect(institution, "Case Western Reserve University") ~ "USA", # Goes to Bolton UK
  ...
  # South Korea:
str_detect(institution, "Hanyang University") ~ "Korea South",
str_detect(institution, "Chungbuk National University") ~ "Korea South",
str_detect(institution, "Chungmugong Leadership Center") ~ "Korea South",
str_detect(institution, "Korea Institute on Behavioral Addictions") ~ "Korea South",
str_detect(institution, "hallym University") ~ "Korea South",
str_detect(institution, "Hallym University") ~ "Korea South",
str_detect(institution, "Seoyoung University") ~ "Korea South",
str_detect(institution, "Yonsei Forest Mental Health Clinic") ~ "Korea South",

# Portugal:
str_detect(institution, "Unity in Multidisciplinary Research on Biomedicine (UMIB)") ~ "Portugal",
str_detect(institution, "Abel Salazar") ~ "Portugal",
str_detect(institution, "IAJ (Gambling Support Institute)") ~ "Portugal",
...
# Hong kong:
 ...
 str_detect(institution, "Pompeu Fabra University") ~ "Spain",
...
str_detect(institution, "Servicio de Psiquiatria, Fundacion") ~ "Spain",
# Italy:
str_detect(institution, "Urbino") ~ "Italy", 
str_detect(institution, "Vincent P. Dole Dual Diagnosis Unit") ~ "Italy", 
...
str_detect(institution, "Anna Nagar") ~ "India",
# Malaysia:
...
str_detect(institution, "Baltalimani Metin Sabanci") ~ "Turkey",
  # Vietnam:
 str_detect(institution, "Nguyen Tat Thanh") ~ "Vietnam",
...
# Now look at papers where we couldn't match the country but linking the world.cities dataset connects a country with a city. Error check this:
 ...
 # 2025-05-04: started with 814 and now checked up to around 400 (start from 350 to be safe)
