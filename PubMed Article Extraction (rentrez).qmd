...
Use the groundhog package to load in relevant packages in a (hopefully) stable form:
...
A simple reminder to look into how feasible it would be to scrape data from the specific gambling journal websites.
:::
...
\`\`\`{r}} \# Clean environment: rm(list = ls())
...
search_results_gambling \<- entrez_search(db="pubmed", term=search_term_gambling, retmax=20000) id_list_gambling \<- search_results_gambling\$ids
...
article_details_gambling \<- map_dfr(chunks, function(ids) { articles \<- entrez_fetch(db="pubmed", id=ids, rettype="medline", retmode="text")

\# Split the articles into individual articles: articles \<- strsplit(articles, "\n\n")\[\[1\]\]

\# Process each article: map_dfr(articles, function(article) { \# Split the article into lines: lines \<- strsplit(article, "\n")\[\[1\]\]
...
```{r}
# Add search URL:
url_gaming_rvest <- "https://pubmed.ncbi.nlm.nih.gov/?term=%28%28%28%28%28%22gaming+disorder%22%5BTitle%2FAbstract%5D%29+OR+%28%22internet+gaming+disorder%22%5BTitle%2FAbstract%5D%29%29+OR+%28%22gaming+addiction%22%5BTitle%2FAbstract%5D%29%29+OR+%28%22video+game+addiction%22%5BTitle%2FAbstract%5D%29%29+OR+%28%22video+game+disorder%22%5BTitle%2FAbstract%5D%29%29+OR+%28%22gaming+dependence%22%29&size=200" # 1569 on 10/07/2023
...
**Notes post-search:** Again, I manually inspected all of the results directly in the PubMed site. All appeared relevant (July 2023).
